const path = require('path');

// ── Load .env (tries __dirname first, then cwd as fallback) ──────────────────
let dotenvResult = require('dotenv').config({ path: path.resolve(__dirname, '.env') });
if (dotenvResult.error) {
  dotenvResult = require('dotenv').config({ path: path.resolve(process.cwd(), '.env') });
}

const express = require('express');
const cors    = require('cors');

const app = express();

// ── Startup diagnostic ────────────────────────────────────────────────────────
console.log('\n── Env check ──────────────────────────────────────');
console.log('  App folder   :', __dirname);
console.log('  Working dir  :', process.cwd());
console.log('  .env loaded  :', dotenvResult.error ? '❌ NOT FOUND' : '✅ OK');
console.log('  DATABASE_URL :', process.env.DATABASE_URL ? '✅ set' : '❌ NOT SET — add it in Vercel dashboard');
console.log('  JWT_SECRET   :', process.env.JWT_SECRET   ? '✅ set' : '⚠️  not set');
console.log('───────────────────────────────────────────────────\n');

// ── DB readiness flag ─────────────────────────────────────────────────────────
let dbReady = false;
let dbError = 'Connecting to database…';

// ── Middleware ────────────────────────────────────────────────────────────────
app.use(cors());
app.use(express.json());
app.use(express.static(path.join(__dirname, 'public')));

// ── Debug page — visit /debug-env in browser to see config status ─────────────
app.get('/debug-env', (_req, res) => {
  const dbUrl = process.env.DATABASE_URL;
  const dbDisplay = dbUrl
    ? '✅ set (' + dbUrl.replace(/:[^:@]+@/, ':***@').slice(0, 60) + '…)'
    : '❌ not set';

  res.send(`
    <html><head><meta charset="utf-8">
    <style>body{font-family:monospace;padding:30px;background:#1a1a2e;color:#e2e8f0}
    h2{color:#6366f1} .ok{color:#22c55e} .err{color:#ef4444} .warn{color:#f59e0b}
    table{border-collapse:collapse;width:100%} td{padding:6px 14px;border-bottom:1px solid #334155}
    </style></head><body>
    <h2>Agency Task Manager — Environment Diagnostics</h2>
    <table>
      <tr><td>DB Ready</td><td class="${dbReady ? 'ok' : 'err'}">${dbReady ? '✅ Connected (Neon PostgreSQL)' : '❌ ' + dbError}</td></tr>
      <tr><td>DATABASE_URL</td><td class="${dbUrl ? 'ok' : 'err'}">${dbDisplay}</td></tr>
      <tr><td>JWT_SECRET</td><td class="${process.env.JWT_SECRET ? 'ok' : 'warn'}">${process.env.JWT_SECRET ? '✅ set' : '⚠️ not set (using fallback)'}</td></tr>
      <tr><td>.env loaded</td><td class="${dotenvResult.error ? 'warn' : 'ok'}">${dotenvResult.error ? '⚠️ No .env file (using Vercel env vars)' : '✅ .env file loaded'}</td></tr>
      <tr><td>App folder</td><td>${__dirname}</td></tr>
      <tr><td>Working dir</td><td>${process.cwd()}</td></tr>
    </table>
    <p style="color:#64748b;margin-top:20px">
      To fix DATABASE_URL: Vercel dashboard → Project → Settings → Environment Variables → add DATABASE_URL.<br>
      Get the connection string from <a style="color:#6366f1" href="https://console.neon.tech" target="_blank">console.neon.tech</a>.
    </p>
    </body></html>
  `);
});

// ── Health-check ──────────────────────────────────────────────────────────────
app.get('/health', (_req, res) => res.json({ status: 'ok', db: dbReady }));

// ── DB-ready guard — all /api routes return 503 JSON until DB is connected ────
app.use('/api', (_req, res, next) => {
  if (!dbReady) {
    return res.status(503).json({
      error: 'Database not connected — ' + dbError,
    });
  }
  next();
});

// ── API Routes ────────────────────────────────────────────────────────────────
app.use('/api/auth',        require('./routes/auth'));
app.use('/api/users',       require('./routes/users'));
app.use('/api/tasks',       require('./routes/tasks'));
app.use('/api/task-types',  require('./routes/taskTypes'));
app.use('/api/roles',       require('./routes/roles'));
app.use('/api/dashboard',   require('./routes/dashboard'));
app.use('/api/time-logs',   require('./routes/timeLogs'));
app.use('/api/performance', require('./routes/performance'));

// ── Serve frontend SPA for all non-API routes ─────────────────────────────────
app.get('*', (_req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'index.html'));
});

// ── Connect to DB with auto-retry ─────────────────────────────────────────────
const { initDatabase } = require('./database/db');
const RETRY_DELAY_MS   = 5000;
const MAX_RETRIES      = 10;

async function connectDb(attempt = 1) {
  try {
    await initDatabase();
    dbReady = true;
    dbError = '';
    console.log('✅ Database connected and ready (Neon PostgreSQL)');
    console.log('📧 Default Admin → dhruv@monkmediaone.com / MMO@1993#');
  } catch (err) {
    dbError = err.message;
    console.error(`❌ DB attempt ${attempt} failed: ${err.message}`);
    if (attempt < MAX_RETRIES) {
      console.log(`🔄 Retrying in ${RETRY_DELAY_MS / 1000}s… (${attempt}/${MAX_RETRIES})`);
      setTimeout(() => connectDb(attempt + 1), RETRY_DELAY_MS);
    } else {
      console.error(`💀 Could not connect after ${MAX_RETRIES} quick attempts.`);
      console.log('⏳ Will retry every 60 s — fix DATABASE_URL in Vercel env vars and it will self-heal.');
      setTimeout(() => connectDb(1), 60_000);
    }
  }
}

// ── Start: listen directly (local) OR export for Vercel serverless ────────────
const PORT = process.env.PORT || 3000;
if (require.main === module) {
  // Running via `node server.js` — local dev
  app.listen(PORT, () => {
    console.log(`🚀 Server listening on port ${PORT}`);
    connectDb();
  });
} else {
  // Imported as a module by Vercel serverless
  connectDb();
}

module.exports = app;
